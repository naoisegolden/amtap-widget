<?php
/**
 * Plugin Name: Amazon Machine Tags Widget
 * Plugin URI: http://wikaina.com/labs/amazon-wordpress-widget
 * Description: A widget for the Amazon Machine Tags plugin
 * Version: 0.1
 * Author: Naoise Golden Santos
 * Author URI: http://goldensantos.com
 *
 * Sample code from http://justintadlock.com/archives/2009/05/26/the-complete-guide-to-creating-widgets-in-wordpress-28
 *
 ...
 * @since 0.1
 */
class Amazon_Widget extends WP_Widget {
	...
	function Amazon_Widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'amazon', 'description' => __('A widget for the Amazon Machine Tags plugin.', 'amazon') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'amazon-widget' );

		/* Create the widget. */
		$this->WP_Widget( 'amazon-widget', __('Amazon Widget', 'amazon'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( $name )
			printf( '<p>' . __('Hello. My name is %1$s.', 'amazon') . '</p>', $name );

		/* If show sex was selected, display the user's sex. */
		if ( $show_sex )
			printf( '<p>' . __('I am a %1$s.', 'amazon.') . '</p>', $sex );
